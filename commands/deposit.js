...
if(money < amount) {
let a = "`"
message.channel.send(`${message.author} Your argument should be either a number and no more than what you have in your wallet (⏣ ${money.toLocaleString()}), or ${a}max${a}`)
} else {
db.add(`bank_${message.author.id}`, amount)
db.subtract(`money_${message.author.id}`, amount)
message.channel.send(`${message.author} **⏣ ${amount}** deposited`)
